...
We will do an exercise using some more complicated information inside data. There is a file containing restaurant information. An array of JSONs that can be easily ingested.
...
That's simple enough but trying to infer the schema could be really tricky. For that it is much easier to use MongoDB's [Compass](https://www.mongodb.com/products/compass) tool that allows to explore the schema interactively.

![](images/schema.png)

From there it can be easily inferred the information contained inside our collection, the percentage of data belonging to each data type inside a field or even the query to filter said information.
...
![](images/schema3.png)
Most of what we have done inside this notebook can be done using Compass in a more visuall manner. Importing documents for example.
...
### How many Chinese or Japanese restaurants?
...
Indexes can be really useful and being an operational database for XXI century applications, text and geospatial searches are key. Imagine that you would like to complement the previous request as follows...

# Find top 10 coffee houses near me

First, we will need what means "near me". That is given my location and restaurant location I need to do a search for those that are at a given distance.

```{r}
r$index(add = '{ "address.coord" : "2d" }')
```

With that index created we can look for documents whose informed location is close to a given location.

```{r}
r$find('{ "address.coord" : { "$geoWithin" : { "$center" : [ [-74, 40.74 ] , 0.01 ] } } }')
```

It certainly is more spectacular when used inside an application that draws maps.

![](images/geo.png)

Now, coffee houses... well anything with the coffee text on it. Text indexes will help us with that. We can look on a single field.
